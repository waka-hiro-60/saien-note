...
export function DetailScreen({ record, onClose, records, tags }) {
  ...
  const handleDelete = async () => {
    try { await records.remove(record.id); onClose(); } catch (e) { console.error(e); }
  };

  const handleArchive = async () => {
    try { await records.toggleArchived(record.id); onClose(); } catch (e) { console.error(e); }
  };
  ...
  return (
    <div style={{
      ...
      background: COLORS.bg, maxWidth: 430, margin: '0 auto',
    }}>
      {/* ヘッダー */}
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '12px 16px', background: headerBg,
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {category === 'veggie' && (
          <>
            <div style={{ position: 'relative', background: '#000' }}>
              {veggieDisplaySrc ? (
                <img src={veggieDisplaySrc} alt="記録写真"
        ...
        {category === 'bed' && (
          <>
            <ImageGrid srcs={editMode ? allImageBase64 : (record.imageBase64s ?? [])}
              editable={editMode} onRemove={handleRemoveImage} />
            {editMode && (
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16, background: COLORS.bedBg }}>
              <DateTimeFields editMode={editMode} date={record.date} time={record.time}
                editDate={editDate} setEditDate={setEditDate} editTime={editTime} setEditTime={setEditTime} />
              {!editMode && <BedVeggieInfo tags={record.tags ?? []} veggieTags={veggieTags} />}
              <TagSection editMode={editMode} tags={record.tags} editTags={editTags}
                allTags={tags.tags} onToggle={toggleEditTag} />
              <CommentSection editMode={editMode} comment={record.comment}
                editComment={editComment} setEditComment={setEditComment} />
          ...
          <>
            ...
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16, background: COLORS.diaryBg }}>
              ...
              <div>
                ...
                {editMode ? (
                  <>
                    <textarea value={editText} onChange={(e) => setEditText(e.target.value.slice(0, 500))} rows={8}
                      style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setConfirmDelete(false)}
                style={{ ...cancelBtnStyle, minHeight: 56, fontSize: 18 }}>キャンセル</button>
...
function DateTimeFields({ editMode, date, time, editDate, setEditDate, editTime, setEditTime }) {
  const fieldStyle = {
    ...
    border: `1px solid ${COLORS.border}`, fontSize: 18,
    background: COLORS.card, color: COLORS.text, boxSizing: 'border-box', minHeight: 48,
  };
  return (
...
function BedVeggieInfo({ tags, veggieTags }) {
  ...
  return (
    <div style={{ padding: '10px 14px', borderRadius: 10, background: '#D0E4D0', border: `1px solid #B0D0B0` }}>
      {places.length > 0 && (
      ...
      {veggies.length > 0 && (
        <div style={{ fontSize: 16, color: COLORS.text }}>栽培中: {veggies.join('、')}</div>
      )}
    </div>
  );
}
